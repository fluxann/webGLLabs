...
function draw() {
    ...
    // let ambientColor = [0.3, 0.0, 0.3]; // Ambient light color
    // let diffuseColor = [0.5, 0.0, 0.5]; // Diffuse light color
    // let specularColor = [0.9, 0.0, 0.9]; // Specular light color
    ...
    gl.uniform2f(shProgram.uCenterLoc, pointU, pointV); // Центр текстури
    gl.uniform1f(shProgram.uAngleLoc, textureRotation);
    surface.Draw();
}
...
function initGL() {
    ...
    shProgram.uCenterLoc = gl.getUniformLocation(prog, 'uCenter');
    shProgram.uAngleLoc = gl.getUniformLocation(prog, 'uAngle');
